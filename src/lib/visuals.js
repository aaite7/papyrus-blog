...
export function injectGlobalStyles() {
  ...
  style.innerHTML = `
    ...
    .popover-btn {
        background: none; border: none; color: #fff; cursor: pointer; font-size: 0.9rem; font-family: sans-serif;
    ...
    /* --- 悬浮操作岛 --- */
    .floating-bar { position: fixed; bottom: 50px; right: 30px; display: flex; flex-direction: column; gap: 15px; z-index: 99999; opacity: 1; transform: translateY(0); pointer-events: auto; }
...
function ensureProgressBar() {
    ...
    if (!bar) {
        ...
        document.body.appendChild(bar); // 挂载到 body 最外层
    }
    return bar;
}
...
export function initSelectionSharer() {
    ...
    if (!popover) {
        ...
        popover.innerHTML = `
            <button class="popover-btn" id="pop-copy">📄 Copy</button>
            <div style="width:1px;background:#555;margin:0 5px;"></div>
            <button class="popover-btn" id="pop-tweet">🐦 Tweet</button>
        `;
        ...
        document.getElementById('pop-tweet').addEventListener('click', () => {
            const text = window.getSelection().toString();
            const url = `https://twitter.com/intent/tweet?text=${encodeURIComponent(text.substring(0, 120) + '...')} via ${encodeURIComponent(window.location.href)}`;
    ...
    document.addEventListener('mouseup', (e) => {
        ...
        if (text.length > 0) {
            ...
            popover.style.top = (rect.top + scrollTop - 50) + 'px'; // 在选区上方50px
            popover.style.left = (rect.left + rect.width / 2 - popover.offsetWidth / 2) + 'px'; // 水平居中
