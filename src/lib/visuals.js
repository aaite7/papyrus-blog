...
export function injectGlobalStyles() {
  ...
  style.innerHTML = `
    ...
    #reading-progress {
        ...
        width: 0%; 
        height: 5px; /* 高度适中 */
        /* 强制使用 Hex 颜色，防止变量未定义导致透明 */
        background: #D4AF37; 
        background: linear-gradient(90deg, #D4AF37, #8B0000); 
        z-index: 2147483647; /* 确保在所有元素之上 */
    ...
    #crop-wrapper { position: relative; display: inline-block; max-width: 100%; border: 2px solid #D4AF37; user-select: none; cursor: crosshair; }
    ...
    /* --- 编辑器 & 代码块 --- */
    .editor-textarea { background-color: #1e1e1e !important; color: #d4d4d4 !important; font-family: 'Consolas', 'Monaco', 'Courier New', monospace !important; font-size: 14px !important; line-height: 1.6 !important; padding: 20px !important; border: 2px solid #D4AF37 !important; border-radius: 6px; white-space: pre !important; overflow-x: auto !important; overflow-y: auto !important; word-wrap: normal !important; tab-size: 4; min-height: 500px; }
    .editor-textarea:focus { outline: none; border-color: #800020 !important; box-shadow: 0 0 15px rgba(212, 175, 55, 0.3); }
    ...
    /* --- 文章内容 --- */
    .article-content { font-size: 1.15rem; line-height: 1.8; color: #333; white-space: pre-wrap !important; overflow-wrap: break-word !important; word-wrap: break-word !important; word-break: break-word !important; max-width: 100%; text-align: left; font-family: 'Lora', 'Consolas', sans-serif; }
    ...
    .article-content th, .article-content td { border: 1px solid #dcdcdc; padding: 8px 12px; }
    ...
    .lightbox-img { max-width: 95%; max-height: 95%; border: 2px solid #D4AF37; box-shadow: 0 0 30px rgba(0,0,0,0.5); transform: scale(0.9); transition: transform 0.3s; }
...
export function updateProgressBar() {
    ...
    const scrollPercent = scrollTotal > 0 ? (scrollTop / scrollTotal) * 100 : 0;
...
export function initSelectionSharer() {
    ...
    if (!popover) {
        ...
        document.getElementById('pop-copy').addEventListener('click', () => { navigator.clipboard.writeText(window.getSelection().toString()).then(() => alert('Copied!')); clearSelection(); });
