...
export function injectGlobalStyles() {
  ...
  style.innerHTML = `
    ...
    * { box-sizing: border-box; }
    ...
    .pinned-badge { display: inline-block; background: #D4AF37; color: #fff; font-size: 0.7rem; padding: 3px 8px; border-radius: 4px; margin-right: 10px; vertical-align: middle; text-transform: uppercase; font-weight: bold; letter-spacing: 1px; transform: translateY(-2px); }

    /* --- >>> 核心修复：悬浮目录 (绝对靠左 + 可见性优化) <<< --- */
    .single-manuscript {
    ...
    #toc {
        position: fixed;
        top: 120px;
        left: 20px; /* >>> 这里的 20px 确保它永远在屏幕最左边 <<< */
        width: 240px;
        max-height: 80vh;
        ...
        text-align: left; /* 左对齐 */
        padding-left: 10px;
        border-left: 2px solid rgba(212, 175, 55, 0.15); /* 左侧分割线 */
        z-index: 1000; /* 提高层级，防止被遮挡 */
        
        opacity: 0;
        animation: fadeInSlide 0.6s ease-out forwards 0.3s;
    }
    ...
    #toc a.active { color: #8B0000; font-weight: bold; transform: translateX(5px); }
    #toc a.active::before { /* 小圆点指示 */
        content: '●'; position: absolute; left: -16px; top: 0; color: #D4AF37; font-size: 0.6rem; line-height: 1.8;
    }
    
    /* >>> 宽松的隐藏策略：只有屏幕非常窄(手机竖屏)才隐藏 <<< */
