...
export function injectGlobalStyles() {
  ...
  style.innerHTML = `
    ...
    /* --- >>> 核心修复 1：找回文章卡片样式 (解决标题超框) <<< --- */
    .manuscript {
        background: #fff;
        padding: 40px; /* 关键：给内容留出呼吸空间 */
        border: 1px solid rgba(212, 175, 55, 0.2); /* 淡淡的金色边框 */
        border-radius: 8px;
        margin-bottom: 40px;
        position: relative;
        box-shadow: 0 4px 20px rgba(0,0,0,0.02); /* 轻微浮起 */
    ...
    #toc {
        ...
        text-align: left; /* 改为左对齐，更符合靠左的视觉习惯 */
        border-right: none;
        border-left: 2px solid rgba(212, 175, 55, 0.1); /* 分割线改到左边 */
        padding-left: 15px; /* 内容离线远一点 */
        z-index: 100;
    ...
    #toc a.active::before { /* 左侧小圆点 */
        content: '●'; position: absolute; left: -21px; top: 0; color: #D4AF37; font-size: 0.6rem; line-height: 1.8;
    }
